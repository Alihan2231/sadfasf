...
class VARPApp:
    ...
    def _create_layout(self):
        ...
        # Logo ve başlık
        logo_frame = tk.Frame(self.sidebar_frame, bg=THEME["sidebar_background"], height=120)
        logo_frame.pack(fill=tk.X, pady=(20, 0))
    
        # PIL ile özel kalkan ikonu oluşturalım
        try:
            from PIL import Image, ImageDraw, ImageTk
        
            # Daha büyük bir ikon boyutu
            icon_size = 60
            icon_image = Image.new('RGBA', (icon_size, icon_size), (0, 0, 0, 0))
            draw = ImageDraw.Draw(icon_image)
        
            # Kalkan şekli çiz (yeşil tonda)
            shield_color = (0, 180, 120, 255)  # Yeşil tonu
            draw.polygon([(icon_size//2, 1), (icon_size-1, icon_size//3), 
                        (icon_size-3, icon_size-2), (icon_size//2, icon_size-1),
                        (3, icon_size-2), (1, icon_size//3)], 
                        fill=shield_color)
        
            logo_icon = ImageTk.PhotoImage(icon_image)
        
            # İkon ve yazıyı grid ile yerleştirelim
            container = tk.Frame(logo_frame, bg=THEME["sidebar_background"])
            container.pack(pady=10)
        
            icon_label = tk.Label(container, image=logo_icon, bg=THEME["sidebar_background"])
            icon_label.image = logo_icon  # Referansı koru
            icon_label.grid(row=0, column=0, padx=(0, 0))  # 4 piksel boşluk
        
            text_label = tk.Label(container, text="V-ARP", font=("Arial", 24, "bold"), 
                               bg=THEME["sidebar_background"], fg=THEME["text_primary"])
            text_label.grid(row=0, column=1, padx=(0, 0), pady=10)
        
        except Exception as e:
            # PIL ile oluşturamadıysak emoji kullanalım
            print(f"Logo oluşturma hatası: {e}")
        
            container = tk.Frame(logo_frame, bg=THEME["sidebar_background"])
            container.pack(pady=10)
        
            icon_label = tk.Label(container, text="🛡️", font=("Arial", 32), 
                               bg=THEME["sidebar_background"], fg=THEME["primary"])
            icon_label.grid(row=0, column=0, padx=(40, 0))  # 4 piksel boşluk
        
            text_label = tk.Label(container, text="V-ARP", font=("Arial", 24, "bold"), 
                               bg=THEME["sidebar_background"], fg=THEME["text_primary"])
            text_label.grid(row=0, column=0, padx=(0, 0), pady=10)
